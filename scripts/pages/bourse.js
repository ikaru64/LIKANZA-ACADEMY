...
function updateScenario(){
  ...
  Object.entries(scenarios).forEach(([key,s])=>{
    ...
    results[key] = prixCible;
    const labels = {defavorable:'Scénario défavorable', central:'Scénario central', favorable:'Scénario favorable'};
    html += `<div class="result-row" style="justify-content:space-between;width:100%;"><span>${labels[key]}</span><span class="mono" style="color:${variation>=0?'var(--emerald)':'var(--bordeaux)'}">${prixCible.toFixed(1)} € (${variation>=0?'+':''}${variation.toFixed(0)}%)</span></div>`;
  });
  ...
  const labels = document.getElementById('scenChartLabels');
  chart.innerHTML=''; labels.innerHTML='';
  const max = Math.max(results.defavorable, results.central, results.favorable, stock.prix);
  [['Actuel', stock.prix], ['Défav.', results.defavorable], ['Central', results.central], ['Favor.', results.favorable]].forEach(([lab,val])=>{
    const bar = document.createElement('div');
    bar.className = 'bar';
    bar.style.height = Math.max(2,(val/max)*100) + '%';
    bar.title = `${lab} : ${val.toFixed(1)} €`;
    chart.appendChild(bar);
    const l = document.createElement('span'); l.textContent = lab; labels.appendChild(l);
  });
}
