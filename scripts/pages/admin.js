...
async function renderNewsPipelineStatus(){
  ...
  const dailyCard = (() => {
    ...
    if(!daily) return `<div class="card"><span class="smallcaps">Récap du jour</span><p style="color:var(--text-dim);font-size:13px;margin-top:8px;">Aucun récap encore généré. Prévu chaque jour à 9h UTC (repli à 15h UTC si le premier passage échoue).</p></div>`;
