...
// neJamaisConclure ajouté pour le sprint de consolidation du 09/09/2026
// (section 46 du prompt d'origine, "Ce qu'il ne faut pas conclure") : le
// pendant explicite du Truth Framework côté lecteur — jusqu'ici, seul
// pourquoi/impact expliquaient ce qu'il FAUT retenir, rien ne bornait
// explicitement la portée réelle de la synthèse.
const CATEGORY_ARTICLE_SCHEMA = {
  ...
  properties: {
    ...
    neJamaisConclure: {type: 'array', items: {type: 'string'}, description: "1 à 3 conclusions hâtives précises qu'un lecteur pourrait être tenté de tirer de cette synthèse mais qu'elle ne permet PAS d'établir (ex: généraliser un seul chiffre à toute une tendance, déduire une cause non confirmée par les titres fournis, extrapoler une évolution de marché future). Toujours formulé comme une mise en garde concrète liée à CETTE synthèse précise, jamais une phrase générique interchangeable d'un article à l'autre."},
  ...
  required: ['titre', 'resume', 'points', 'pourquoi', 'impact', 'aSurveiller', 'neJamaisConclure', 'accordSources', 'sourcesUsed']
};
...
- Un champ "neJamaisConclure" : 1 à 3 conclusions hâtives précises qu'un lecteur pressé pourrait être tenté de tirer de CETTE synthèse précise mais qu'elle ne permet pas d'établir (par exemple : généraliser un seul chiffre ponctuel à toute une tendance, attribuer une cause qui n'est pas confirmée par les titres fournis, ou extrapoler une évolution de marché future à partir d'un seul événement passé). Chaque conclusion à ne pas tirer doit être concrètement liée au contenu de cette synthèse, jamais une mise en garde générique qui irait tout aussi bien sur n'importe quel autre article.
${multiSource
...
async function generateCategoryArticle(categorie, articles, distinctSourceNames){
  ...
  return {
    ...
    neJamaisConclure: Array.isArray(parsed.neJamaisConclure) ? parsed.neJamaisConclure.map(p => String(p).trim()).filter(Boolean) : [],
