...
/* ------------------------------------------------------------
   Extension multi-pays (terminal économique, 06/09/2026) : même série
   HICP, geo paramétrable. "U2" = zone euro agrégée (le VRAI code BCE
   pour la zone euro, déjà utilisé par le taux de dépôt ci-dessus) — à ne
   pas confondre avec "EU27_2020" (Union européenne à 27) utilisé par les
   agrégats Eurostat de lib/eurostat.js pour le chômage/PIB/dette, qui eux
   ne supportent aucun code zone euro (vérifié en direct, dimension geo
   vide). Les deux agrégats sont réels mais de périmètre différent —
   toujours étiquetés distinctement côté front, jamais confondus.
   ------------------------------------------------------------ */
const ECB_INFLATION_COUNTRY_LABELS = {FR: 'France', DE: 'Allemagne', IT: 'Italie', ES: 'Espagne', U2: 'Zone euro'};

async function fetchEcbInflation(geo){
  const label = ECB_INFLATION_COUNTRY_LABELS[geo] || geo;
  const url = `https://data-api.ecb.europa.eu/service/data/ICP/M.${geo}.N.000000.4.INX?format=jsondata&startPeriod=2016-01`;
  const points = await fetchEcbSeriesFull(url, `inflation ${geo}`);
  return {
    points,
    source: 'Banque centrale européenne (BCE) — statistiques HICP',
    sourceUrl: 'https://data.ecb.europa.eu/data/datasets/ICP',
    seriesKey: `ICP.M.${geo}.N.000000.4.INX`,
    instrument: `Indice des prix à la consommation harmonisé (IPCH), ${label}, base 100`,
    frequency: 'Mensuelle'
  };
}

const fetchEcbInflationFR = () => fetchEcbInflation('FR');
...
module.exports = { fetchEcbDepositRate, fetchEcbInflationFR, fetchEcbMortgageRateFR, fetchEcbHomePriceIndexFR, fetchEcbDepositRateHistory, fetchEcbInflation, ECB_INFLATION_COUNTRY_LABELS };
