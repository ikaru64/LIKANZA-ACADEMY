...
async function fetchYahooQuote(entry, opts){
  ...
  // Paires (date, clôture[, volume]) alignées, sans les trous (séances sans
  // clôture). Le volume, lui, n'est jamais un critère de filtrage : une
  // séance avec clôture valide mais volume absent reste conservée, avec
  // volume undefined — jamais un 0 fabriqué à la place d'une donnée manquante.
  const sessions = timestamps
    .map((t, i) => ({date: new Date(t * 1000).toISOString(), close: closes[i], volume: typeof volumes[i] === 'number' ? volumes[i] : undefined}))
