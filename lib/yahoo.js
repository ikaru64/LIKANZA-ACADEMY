...
// (marges, croissance, valorisation...) en un seul appel Yahoo quoteSummary —
// même mécanisme cookie+crumb que ci-dessus, un crumb peut être partagé entre
// plusieurs appels (voir api/company-profile.js, mode batch) pour limiter le
// nombre de requêtes contre cet endpoint non documenté et fragile.
async function fetchCompanyProfile(symbol, crumbCtx){
  ...
  return {
    ...
    fundamentals: {asOfDate: new Date().toISOString(), source: 'Yahoo Finance (quoteSummary)', fields}
  };
}
