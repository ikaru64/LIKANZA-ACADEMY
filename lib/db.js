...
async function ensureDailyNewsTable(sql){
  ...
  `;
  // items : chantier "vraies sources par article" — le récap devient une
  // liste de 3 à 5 actualités distinctes, chacune avec ses propres sources
  // réellement citées et sa corroboration calculée (voir
  // lib/gemini.js:filterSourcesUsed), plutôt qu'un unique résumé flou et un
  // pool de sources indifférencié. title/summary/sources restent en place
  // pour compatibilité (dérivés en code depuis items, voir
  // api/generate-daily-news.js), jamais une seconde colonne qui divergerait.
  await sql`ALTER TABLE daily_news ADD COLUMN IF NOT EXISTS items JSONB NOT NULL DEFAULT '[]'`;
}
...
async function ensureWeeklyNewsTable(sql){
  ...
  // ne_jamais_conclure : sprint de consolidation du 09/09/2026, même
  // discipline de migration ALTER (pas seulement CREATE) que les deux
  // colonnes juste au-dessus.
  await sql`ALTER TABLE weekly_news ADD COLUMN IF NOT EXISTS ne_jamais_conclure JSONB NOT NULL DEFAULT '[]'`;
}
