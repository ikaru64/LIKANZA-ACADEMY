/* ============================================================
   ...
   26/08/2026 : francetvinfo.fr redirige désormais (301) vers
   franceinfo.fr (migration de domaine côté éditeur, constatée en
   diagnostiquant un récap quotidien resté figé plusieurs jours) — les 5
   URLs concernées mises à jour vers le nouveau domaine, chacune
   revérifiée en direct (HTTP 200, contenu daté du jour) avant ce
   correctif. fetch() suit les redirections HTTP par défaut, donc
   l'ancien domaine n'aurait pas dû casser la génération à lui seul,
   mais dépendre indéfiniment d'un 301 reste fragile (un éditeur peut
   retirer la redirection sans préavis) — corrigé à la source plutôt
   que laissé en l'état.
   ============================================================ */

// Flux mélangés utilisés pour le récap quotidien (toutes thématiques
// confondues). Élargi (3 -> 5 flux, 3 éditeurs distincts au lieu de 2) après
// retour utilisateur : avec seulement 2 flux Investing.com + 1 flux BFM, les
// 2/3 des sources venaient mécaniquement du même éditeur, ce qui ne donne pas
// une vraie impression de "plusieurs journaux" même quand le nombre total de
// sources semble élevé. Franceinfo — Entreprises et Franceinfo — Éco/Conso
// sont réutilisés tels quels depuis WEEKLY_CATEGORY_FEEDS ci-dessous (déjà
// vérifiés en direct), aucun nouveau flux non testé ajouté ici.
const RSS_FEEDS = [
  ...
  {name:'Franceinfo — Entreprises', url:'https://www.franceinfo.fr/economie/entreprises.rss'},
  {name:'Franceinfo — Éco/Conso', url:'https://www.franceinfo.fr/economie.rss'}
];
...
const WEEKLY_CATEGORY_FEEDS = [
  ...
  {categorie: 'Géopolitique', slug: 'geopolitique', feeds: [
    ...
    {name: 'Franceinfo — Monde', url: 'https://www.franceinfo.fr/monde.rss'}
  ]},
